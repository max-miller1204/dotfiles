...
- The pi coding agent installs through fnm-managed npm into the stable `~/.local/share/npm-pi` prefix (`run_onchange_before_18-install-pi.sh.tmpl`, `latest` channel), with its CLI linked into `~/.local/bin`.
  ...
  `subagents.watchdog` is enabled with `openai-codex/gpt-5.6-sol` at thinking high.
  Note what that costs: the fork's `/subagents-watchdog recommend-model` deliberately returns a model from a DIFFERENT family than the session's, so a watchdog on `defaultModel` reviews with the same blind spots as the session that wrote the code, and its independence comes only from fresh context plus an adversarial prompt.
  Moving it to `openai-codex/gpt-5.6-terra` would restore model independence at the cost of reviewer strength; that tradeoff is the live question if watchdog reviews start reading as agreeable.
  `verify.sh` therefore gates that `main.model` AND `main.thinking` are both set rather than gating independence, because a watchdog model configured without a thinking level silently runs with thinking off.
