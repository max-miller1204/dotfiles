...
- The pi coding agent installs through fnm-managed npm into the stable `~/.local/share/npm-pi` prefix (`run_onchange_before_18-install-pi.sh.tmpl`, `latest` channel), with its CLI linked into `~/.local/bin`.
  ...
  Setting a thinking level a model does not support fails silently in the one direction that matters, which is why `check-pi-subagent-config.sh` validates every pinned level against the generated models store rather than merely asserting the field is present.
  That script owns the structural invariants too - package name, the Anthropic ban, tier coverage across the builtin roster, watchdog model/thinking, watchdog provider independence, and `modelScope` enforcement - and CI runs it against the source tree on every PR while the E2E runs the SAME script against the applied `~/.pi` tree with the models store attached.
