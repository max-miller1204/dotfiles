...
- Tool-owned config is generated by each tool's own CLI, not hand-vendored into chezmoi-managed files.
  Claude plugins/marketplace come from the `claude plugin` CLI, the brev skill from `brev agent-skill`, and the herdr agent hook from `herdr integration install`.
...
## herdr agent integration state

- herdr's claude integration is the SECOND block chezmoi strips from `~/.claude/settings.json` each apply, and `run_after_66-herdr-agent-integrations.sh.tmpl` re-asserts it with `herdr integration install claude` for the same reason `run_after_65` re-asserts the plugins - so it is a plain `run_after_`, never a `run_onchange_after_`.
  The integration has two halves and only one of them is chezmoi's business: the hook SCRIPT at `~/.claude/hooks/herdr-agent-state.sh` (unmanaged, an apply never touches it) and a `SessionStart` entry inside the managed `settings.json` that invokes it.
  An apply strips only the second half, and that half is what makes a Claude Code session report its session id to the herdr server; without it herdr stores the pane with no `agent_session` and restores it as a bare shell instead of resuming the agent.
- `herdr integration status` is NOT a valid check for this and must never be used as the script's guard: it reads the version marker inside the unmanaged hook script, so it reports `claude: current` for an install whose settings wiring is gone.
  Assert the wiring itself - `run_after_66` re-runs the (idempotent, single-entry) installer and then queries the applied `settings.json` for the exact hook command.
  `verify.sh` gates that exact wiring post-apply and asserts that Atuin's existing `PreToolUse`, `PostToolUse`, and `PostToolUseFailure` hook commands remain present.
- Only agents whose herdr wiring lands in a chezmoi-managed file belong in that script, which today is claude alone.
  pi and opencode drop their integration into auto-discovered directories (`~/.pi/agent/extensions`, `~/.config/opencode/plugins`) that are managed WITHOUT an `exact_` prefix, so chezmoi leaves the herdr file beside the managed ones, and codex writes its own `~/.codex` hooks file, which chezmoi does not manage as a target at all.
  Adding one of those to the script would be churn, not safety - extend it only once an apply is shown to actually strip that agent's wiring.
...
- `~/.claude/settings.json` is intentionally CLI-owned for plugins, and it drifts after every `chezmoi apply` by design.
  ...
  Pi installs through fnm-managed npm into the stable `~/.local/share/npm-pi` prefix via `run_onchange_before_18-install-pi.sh.tmpl` (`latest` channel, `~/.local/bin/pi` launcher), and Hunk is installed the same way into a stable native prefix.
