...
- The on-demand E2E workflow (`.github/workflows/e2e-native-ubuntu.yml`) runs the real bootstrap (`chezmoi init --apply --promptDefaults`) on a clean GitHub-hosted Ubuntu VM and verifies the installed end state with `.github/e2e/verify.sh`.
  ...
  `check-pi-subagent-config.sh` follows that same shape for pi's subagent settings and takes an OPTIONAL second argument, pi's generated models store: CI omits it and runs the structural invariants alone, while the E2E passes the applied store so every pinned thinking level is additionally validated against what the model really supports.
  Split a check that way whenever part of it needs machine-only state - putting the whole thing in `verify.sh` to reach that state costs PR coverage for the structural half, which is exactly how an invariant ends up enforced nowhere a PR can see.
