...
- The on-demand E2E workflow (`.github/workflows/e2e-native-ubuntu.yml`) runs the real bootstrap (`chezmoi init --apply --promptDefaults`) on a clean GitHub-hosted Ubuntu VM and verifies the installed end state with `.github/e2e/verify.sh`.
  ...
  That TTY-less second apply must use `--force` because the first apply's Claude plugin and herdr setup intentionally mutate the managed settings file; this explicitly selects the source version before the always-run scripts re-assert their tool-owned state.
  ...
  `check-pi-model-pins.sh` takes an OPTIONAL third argument, pi's generated models store: CI omits it and validates model pins and thinking level NAMES alone, while the E2E passes the applied store so every pinned level is additionally checked against what its model really supports - each level against every model it reaches, fallbacks included.
  Splitting those two apart is the point: whether `mediun` is a level at all needs no machine state, so it fails on a PR, while whether `kimi-k3` supports `high` needs the catalog and waits for the E2E.
  pi writes that store only when a configured provider refreshes its catalog, so the E2E runs the pin check AFTER `pi_native_runtime_smoke` (the apply's first real pi invocation - `pi --version` in the install script creates no `~/.pi` at all) and passes the store only when the file exists.
  Keep both properties on any check that reaches for machine-only state: order it after whatever produces that state, and degrade to the checks that do not need it rather than hard-failing a runner that never produces it.
  Split a check that way whenever part of it needs machine-only state - putting the whole thing in `verify.sh` to reach that state costs PR coverage for the rest, which is exactly how an invariant ends up enforced nowhere a PR can see.
  `check-pi-subagent-config.sh` follows the same shared shape for pi's structural subagent settings and needs no machine state at all.
  `test-pi-config-checks.sh` is the regression coverage for BOTH, and CI and the E2E each run it: a checker that has silently stopped catching a violation prints the same pass line as a working one, so running either against the committed configuration alone proves nothing.
  It invokes the real scripts over mutated copies of the committed settings and agent definitions and asserts exit status and the emitted finding - never the checkers' source text - and ships a small fixture models store so the thinking-level cases run on a PR, where no generated store exists.
  Any new invariant in either checker needs a fixture here that violates exactly it.
  Thinking levels deliberately live in the pin checker rather than there: a level is only meaningful beside its model, both appear in agent frontmatter as well as settings, and splitting them would mean a second script re-parsing the same frontmatter.
