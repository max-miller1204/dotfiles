...
- The on-demand E2E workflow (`.github/workflows/e2e-native-ubuntu.yml`) runs the real bootstrap (`chezmoi init --apply --promptDefaults`) on a clean GitHub-hosted Ubuntu VM and verifies the installed end state with `.github/e2e/verify.sh`.
  ...
  `check-pi-model-pins.sh` takes an OPTIONAL third argument, pi's generated models store: CI omits it and validates model pins alone, while the E2E passes the applied store so every pinned thinking level is additionally checked against what its model really supports - each level against every model it reaches, fallbacks included.
