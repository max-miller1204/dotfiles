...
Subagents come from `npm:pi-subagents`, the [nicobailon fork](https://github.com/nicobailon/pi-subagents), which ships `scout`, `researcher`, `worker`, `reviewer`, `delegate`, `context-builder`, `planner`, `oracle`, and `advisor` as built-ins and reads their routing from the `subagents.agentOverrides` block of `agent/settings.json`.
...
The subagent watchdog is enabled as an adversarial reviewer of repo edits on `kimi-k3` at high thinking, deliberately on a different provider than the main session so it does not review with the same blind spots as the code's author.
