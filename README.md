...
**pi config ownership.**
pi's config lives under `~/.pi` (source: `dot_pi/`): `agent/settings.json` (models, subagent routing, and published extension packages, including `npm:pi-git-diff` and `npm:pi-subagents`), `agent/agents/` (the `shaper` subagent definition), `agent/extensions/` (the custom status bar, GitHub issue `#` autocomplete, `/handoff`, and Treehouse worktree guard extensions), `agent/prompts/` (prompt templates such as `/artifact`), and `web-search.json` (provider choice), plus the rendered `agent/mcp.json` above.
...
Subagents come from `npm:pi-subagents`, the [nicobailon fork](https://github.com/nicobailon/pi-subagents), which ships `scout`, `researcher`, `worker`, `reviewer`, `delegate`, `context-builder`, `planner`, and `oracle` as built-ins (plus `advisor` as an alias of `oracle`) and reads their routing from the `subagents.agentOverrides` block of `agent/settings.json`.
Those built-ins replaced the hand-maintained `Explore.md` and `Plan.md` definitions, which `.chezmoiremove` deletes from `~/.pi/agent/agents/`.
Routing follows the fork's recommended four capability tiers rather than one model everywhere: `gpt-5.6-luna` at low thinking for recon, `gpt-5.6-terra` at medium for well-scoped implementation and review, `gpt-5.6-sol` at high for hard work that arrives with explicit completion criteria, and `minimax-m3` for the intent tier, whose `shaper` agent handles ambiguous design, product, and planning work and falls back to the other provider when an `opencode-go` limit is hit.
The subagent watchdog is enabled as an adversarial reviewer of repo edits on `kimi-k3` at max thinking, deliberately on a different provider than the main session so it does not review with the same blind spots as the code's author.
Only `openai-codex` and `opencode-go` models are used anywhere in pi's config, because those run against a subscription while Anthropic models bill the API; `subagents.modelScope` enforces that as an allowlist rather than leaving it to convention.
`.github/scripts/check-pi-model-pins.sh` keeps every model pinned in agent frontmatter or in the `subagents` settings block inside the `enabledModels` allowlist.
The Git diff viewer is installed from npm rather than duplicated under `agent/extensions/`.
Hunk is installed from npm into `~/.local/share/npm-hunkdiff`, and Pi loads its bundled review skill from that stable prefix as declared in `agent/settings.json`.
...
- `.chezmoidata/packages.yaml` - single source of truth for native bootstrap packages.
...
- `dot_pi/` - the rest of the pi coding agent config: `agent/settings.json`, `agent/agents/` (subagent definitions), `agent/extensions/`, `agent/prompts/`, `web-search.json` (pi's runtime state is not managed - see [Agents (multi-agent)](#agents-multi-agent))
