...
Routing follows the fork's recommended four capability tiers rather than one model everywhere: `gpt-5.6-luna` at low thinking for recon, `gpt-5.6-terra` at medium for well-scoped implementation and review, `gpt-5.6-sol` at high for hard work that arrives with explicit completion criteria, and `claude-fable-5` for the intent tier, whose `shaper` agent handles ambiguous design, product, and planning work and falls back across providers when an Anthropic limit is hit.
The subagent watchdog is enabled as an adversarial reviewer of repo edits on `claude-opus-4-8` at high thinking, deliberately on a different provider than the main session so it reviews independently.
