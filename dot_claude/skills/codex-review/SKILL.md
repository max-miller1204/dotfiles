...
>
> **Sanity-check that Codex actually read the LOCAL repo.** This is an orchestrator judgment call - you read the verdict text and decide whether Codex genuinely read the local files - not a mechanical keyword grep. Hard-stop and report ONLY when the critique reports Codex's OWN inability to read: it explicitly says it could not read the plan or the repo files, that its shell/command sandbox failed before it could run a read, or that it reviewed the repo via GitHub / a connector / "the indexed version" instead of the local file. Also hard-stop when the critique shows no concrete evidence of a local read at all: no `file:line` citations and no quotes of actual local/uncommitted content. Any of those means the sandbox failed to initialize or Codex fell back to its stale indexed view, so the verdict is worthless (this is exactly what `-c features.use_legacy_landlock=true` prevents; confirm the flag is present), and you must NOT continue the loop on it. **Do not false-positive on the sandbox topic.** The mere appearance of words like `bwrap`, `sandbox`, `loopback`, or "Operation not permitted" in the critique is NOT proof of failure. When the plan under review is itself about sandboxing - a common and expected case for this skill; this skill's own plan was reviewed exactly that way - a legitimate critique that read the repo fine will naturally quote those exact terms. Judge by whether Codex reports failure to read ITS OWN environment, not by which words the critique contains. The strong positive signal it read locally is unchanged: the critique cites exact `file:line` locations for your uncommitted changes, or quotes actual local content, both absent from any GitHub view. This check is mandatory after EVERY round, resumes included (each `codex exec resume` re-establishes the sandbox and can independently fall back) - it is step 2 of the per-round checklist below.
...
2. Verify Codex actually read the LOCAL repo (mandatory EVERY round, resumes included). This is a judgment call, not a keyword grep: read the critique in `$CR_DIR/verdict.txt` and confirm it shows evidence of a real local read - it cites exact `file:line` locations, or quotes actual local/uncommitted content, that no GitHub view has. Hard-stop and report ONLY when Codex reports its OWN inability to read: it explicitly says it could not read the plan/files, that its shell/command sandbox failed before it could run a read, or that it reviewed the repo via GitHub / a connector / "the indexed version" instead of the local file - or when there is no concrete evidence of a local read at all (no `file:line` citations, no quotes of local content). Do NOT treat the mere appearance of `bwrap`/`sandbox`/`loopback`/`Operation not permitted` in the critique as proof of failure: when the plan under review is about sandboxing, a legitimate critique quotes those exact terms while having read the repo fine. A genuine fallback means the verdict is worthless - do not continue the loop. Each `codex exec resume` re-establishes the sandbox, so this can regress on any round, not just round 1; the fuller explanation is under the Round 1 block above.
